...
export interface InitProfileParseFileSample {
  ...
  /** Present for C/C++ files that attempted declaration-macro recovery. */
  macroRecovery?: {
    invocationCount: number;
    recoverySourceBytes: number;
    auxiliaryParseAttempts: number;
    auxiliaryParseMs: number;
...
const PROFILE_ENV_KEYS = [
  ...
  'CODEGRAPH_NO_DECLARATION_MACRO_SHARDING',
        ...
        this.parseWindowStateSamples > 0
          ? {
              ...
              slowestFiles: this.slowestParseFiles.map((sample) => ({
                ...
                macroRecovery: sample.macroRecovery
                  ? {
                    ...sample.macroRecovery,
                    auxiliaryParseMs: roundMs(sample.macroRecovery.auxiliaryParseMs),
                  }
                  : undefined,
              })),
            }
