...
## codegraph 源码探索协议
本仓库已建立 codegraph 索引（\`.codegraph/\`）。探索已索引源码时，优先使用 codegraph 获取完成任务所需的**最小充分上下文**，不要把 \`Read\` 原样替换成整文件或连续文件窗口。本节为仓库专用规则；与通用工具说明冲突时，以本节为准。
### 1. 按已知信息直接选择入口
不要执行固定的“目录 → 大纲 → 符号”流水线：
- 已知目录、不知道文件：\`codegraph_wx_files(path=<目录>)\`。
- 已知文件、不知道符号：\`codegraph_wx_node(file=<文件>, symbolsOnly=true, outlineQuery=<可选名称片段>, outlineLimit<=30)\`。
- 已知名称/复制来的函数签名，但不确定位置或重载：一个目标用 \`codegraph_wx_search(query=<名称或完整签名>, includeCode="if_unique")\`；2–8 个名称用一次 \`codegraph_wx_search(queries=[...])\`，禁止循环单查询。
- 已知精确符号和位置：\`codegraph_wx_node(symbol=<符号>, file=<可选文件>, line=<可选行>, includeCode=true, includeRelations=false)\`。
- 已知 1–8 个精确实现目标：优先一次 \`codegraph_wx_node(targets=[...])\` 原生批量；它与 \`codegraph_wx_context(targets=[...])\` 使用同一个 implementation bundle。不要循环单目标 \`codegraph_wx_node\`。
- 查调用方/被调用方/影响：\`codegraph_wx_callers\` / \`codegraph_wx_callees\` / \`codegraph_wx_impact\`。
...
\`\`\`
规则：
- 同一问题中已经明确的符号、成员、文本锚点和 file regions 必须尽量合并到一次 \`codegraph_wx_node(targets=[...])\`；\`codegraph_wx_context(targets=[...])\` 是等价入口。
...
### 4. \`node\` 模式与自动纠正
\`codegraph_wx_node\` 只使用以下模式：
\`\`\`text
# 符号源码
codegraph_wx_node(symbol=..., file=可选, line=可选, includeCode=true, includeRelations=false)
# 文件符号大纲
codegraph_wx_node(file=..., symbolsOnly=true, outlineQuery=可选, outlineLimit<=30)
# 单个精确文件窗口
codegraph_wx_node(file=..., offset=..., limit<=500)
...
对已经精确消歧的 C++ 虚函数/override，\`codegraph_wx_callers\` 会沿索引中的 virtual-dispatch family 聚合基类声明和派生实现两端的调用方；若结果提示已扩展 dispatch family，不要再用 Grep 搜基类调用。
结果足以支持当前判断时立即停止，不要打开全部调用链节点，也不要用 \`grep\` 复核。
### 6. \`text_search\` 与原生工具回退
- 不知道精确文件的宏、注册字符串、表名、日志文本：把多个字面量合并到一次 \`codegraph_wx_text_search\`，\`path\` 使用最窄源码目录，不得使用仓库根目录。
...
错误：
\`\`\`text
codegraph_wx_search(query="get_row")
codegraph_wx_node(symbol="get_row", includeCode=true)
\`\`\`
...
\`\`\`
错误：对四个成员循环四次单目标 \`codegraph_wx_node\`。
