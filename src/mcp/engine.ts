...
export class MCPEngine {
  ...
  // (or null forever if no .codegraph-wx/ ever turned up — that's a valid
  ...
  /**
   * Walk up from `searchFrom` to find the nearest `.codegraph-wx/` and open it.
  ...
  private async doInitialize(searchFrom: string): Promise<void> {
    ...
    const resolvedRoot = findNearestCodeGraphRoot(searchFrom);
    if (!resolvedRoot) {
      // No .codegraph-wx/ above searchFrom. Sessions may still discover one later via roots/list
