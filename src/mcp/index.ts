 ...
 *   `.codegraph-wx/` is reachable, or the daemon machinery fails for any reason.
 ...
 * `null` when no `.codegraph-wx/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.codegraph-wx/`.
 *
 ...
 * `.codegraph-wx/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
...
export class MCPServer {
   ...
   *   3. No `.codegraph-wx/` reachable → direct mode (the daemon's lockfile and
   *      socket both live under `.codegraph-wx/`).
  ...
  async start(): Promise<void> {
    ...
    if (!root) {
      ...
      return this.startDirect('no .codegraph-wx/ root found');
    }
