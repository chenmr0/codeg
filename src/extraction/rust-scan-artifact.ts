/** Release metadata for optional standalone helpers, not a compiler at install time. */
...
export interface RustScanManifest {
  schema: number; protocol: number; platform: string; arch: string; target: string;
  executable: string; packageVersion: string; sourceHash: string; sha256: string;
  profile: string;
...
export function rustScanPackageVersion(): string {
  return (JSON.parse(fs.readFileSync(path.join(__dirname, '../../package.json'), 'utf8')) as { version: string }).version;
}
/** Check the actual executable, not just a boolean left by an older build. */
export function checkRustScanArtifact(binary: string, platform: string = process.platform,
  arch: string = process.arch, version = rustScanPackageVersion(), requireValidated = true): RustScanManifest {
  const spec = RUST_SCAN_TARGETS[`${platform}-${arch}` as keyof typeof RUST_SCAN_TARGETS];
  ...
  let manifest: RustScanManifest;
  try { manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8')) as RustScanManifest; }
  catch { throw new Error('manifest-invalid'); }
  if (!manifest || manifest.schema !== 1 || manifest.protocol !== RUST_SCAN_PROTOCOL ||
  ...
  if (sha256(fs.readFileSync(binary)) !== manifest.sha256) throw new Error('binary-checksum');
  const validation = manifest.validation;
  if (requireValidated && (!validation || validation.suite !== RUST_SCAN_VALIDATION_SUITE ||
