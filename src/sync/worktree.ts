 ...
 *
 * A CodeGraph index lives in a `.codegraph-wx/` directory and is resolved by
...
export interface WorktreeIndexMismatch {
  ...
  worktreeRoot: string;
  /** The (different) working tree whose `.codegraph-wx` index is being used. */
  indexRoot: string;
}
 ...
 *     that merely happens to contain a `.codegraph-wx/`), which keeps non-git
